    ...
    if(Record[now+1][0]!=0)
    {
        for(a=1,b=0,c=0;a<=Record[now+1][0];a++)
            if(b<Num[Record[now+1][a]]&&Pan[Record[now+1][a]]==0)
                {b=Num[Record[now+1][a]];c=Record[now+1][a];}
        Set(c);Solve(now+1);
    }
}
...
int main()
{
    ...
    {
        scanf("%d %d",&a,&b);
        Link[a][0]++;Link[a][Link[a][0]]=b;
        Link[b][0]++;Link[b][Link[b][0]]=a;
    }
    Num[1]=Search(1,1);
    Record[1][0]=1;Record[1][1]=1;
    Tree(1);memset(Pan,0,sizeof(Pan));
    Solve(1);Pan[0]=0;
    for(a=1;a<=N;a++)
        if(Pan[a]==0)
            Ans++;
